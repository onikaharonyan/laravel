...
function wowAnimation() {
    ...
    $( '#BrandsSelect' ).on ( 'change' , function () {
        ...
        $.ajax ({
            ...
            dataType: 'json',
            success : function ( e ) {
                $( '#ModelsAppend' ).html ( '<option value="">Select Model</option>' );
                $.map ( e.result ,  function ( model ) {
                    $( '#ModelsAppend' ).append ( `
                        <option value="`+model.id+`">`+model.title+`</option>
                    ` );
                });
                $('#ModelsAppend').niceSelect('destroy');
                $('#ModelsAppend').niceSelect();
            },
        });
    });

    if ( $( '#ModelsAppend' ).attr ( 'data-load' ) !== '' ) {
        let value = $( '#ModelsAppend' ).attr ( 'data-load' )
        let active = $( '#ModelsAppend' ).attr ( 'data-active' )

        $.ajax ({
            url : $( '#ModelsAppend' ).attr ( 'data-link' ),
            data: { 'id' : value },
            type : 'post',
            dataType: 'json',
            success : function ( e ) {
